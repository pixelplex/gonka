...
// staleLeaseStore abstracts *validationlease.Store for testing.
...
func (r *RetryLoop) retryForEscrow(ctx context.Context, escrowID string) {
	for {
		...
		if r.phase != nil && r.phase.EpochID() > leaseEpochID {
			...
			if err := r.leases.SetResult(ctx, escrowID, inferenceID, validationlease.StatusSkipped); err != nil {
...
func (r *RetryLoop) retryOne(ctx context.Context, escrowID string, inferenceID, epochID uint64) error {
	...
	if err := r.leases.SetResult(ctx, escrowID, inferenceID, validationlease.StatusSubmitted); err != nil {
