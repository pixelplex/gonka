...
func main() {
	...
	chainClient, err := chain.New(nodeConfig.GrpcUrl)
	if err != nil {
		log.Fatalf("chain client: %v", err)
	}

	recorder, err := newChainIdentity(chainClient, apiAccount, kr)
	if err != nil {
		log.Fatalf("chain identity: %v", err)
	}
	...
	br := bridge.NewChainBridge(chainClient, nil)
	...
	eng := inference.NewEngine(mlClient, payloadStore, httpClient, chainParams)
	validator := inference.NewValidator(mlClient, httpClient, br, recorder, eng, chainParams, devshardtypes.NormalizeSessionVersion(runtimeVersion))
	...
	manager := session.NewHostManager(store, signer, eng, validator, devshardtypes.NormalizeSessionVersion(runtimeVersion), br, payloadStore, recorder)
	if err := manager.RecoverSessions(); err != nil {
		slog.Warn("recover sessions failed", "error", err)
	}

	e := buildServer(chainClient)
	manager.Register(e.Group(devshardpkg.LegacyRoutePrefix))
...
func loadNodeConfigFromEnv() ChainNodeConfig {
	...
	return ChainNodeConfig{
		...
		GrpcUrl:         envOr("NODE_GRPC_URL", nodeHost+":9090"),
...
func newChainParamsProvider(ctx context.Context, recorder inference.PayloadAuthClient) *chainParamsProvider {
