...
func (h *Handlers) GetModels(ctx echo.Context) error {
	...
	activeModels := make([]gen.RawProtoJson, 0, len(parentEpochData.SubGroupModels))
	...
	return ctx.JSON(http.StatusOK, gen.ModelsResponse{
		Models: activeModels,
	})
}
...
func (h *Handlers) GetGovernanceModels(ctx echo.Context) error {
	...
	models := make([]gen.RawProtoJson, len(modelsResponse.Model))
	for i := range modelsResponse.Model {
		models[i] = &modelsResponse.Model[i]
	}

	return ctx.JSON(http.StatusOK, gen.GovernanceModelsResponse{
		Models: models,
	})
}
...
func (h *Handlers) GetGovernanceModelsLegacy(ctx echo.Context) error {
	...
	return ctx.JSON(http.StatusOK, gen.ModelsLegacyResponse{
		Model: models,
	})
}
	...
	for _, modelId := range parentEpochData.SubGroupModels {
		...
		if modelEpochData.EpochGroupData.ModelSnapshot != nil {
			...
			models = append(models, gen.ModelPriceDto{
				Id:                     m.Id,
				UnitsOfComputePerToken: int(m.UnitsOfComputePerToken),
				PricePerToken:          int(pricePerToken),
			})
		}
	}

	return ctx.JSON(http.StatusOK, gen.PricingDto{
		UnitOfComputePrice: int(unitOfComputePrice),
		Models:             models,
	})
}
...
func (h *Handlers) GetGovernancePricing(ctx echo.Context) error {
	...
	for i, m := range modelsResponse.Model {
		...
		modelDto := gen.ModelPriceDto{
			Id:                     m.Id,
			UnitsOfComputePerToken: int(m.UnitsOfComputePerToken),
			PricePerToken:          int(legacyPricePerToken),
		}
		...
		if dynamicPricingEnabled {
			if dynamicPrice, exists := dynamicPrices[m.Id]; exists {
				modelDto.PricePerToken = int(dynamicPrice)
			}
