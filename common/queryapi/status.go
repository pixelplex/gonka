...
// Change: error response uses echo.HTTPError ({"message":"..."}) instead of the
// original ad-hoc JSON body ({"error":"..."}) to match the rest of this API.
func (h *Handlers) GetVersions(ctx echo.Context) error {
	...
	if err != nil {
		...
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to get node info")
	}
